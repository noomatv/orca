...
module.exports = {
  ...
  replaceFrontMatter: function(mdContent, newFrontMatter) {
    // Take out front matter to get mdBody
    // return newFrontMatter + mdBody
    return "Hello"
  },

  deleteFromS3AndCleanFrontMatter: function(mdFilePath, s3Data, client, sender, callback) {
    var self = this
    fs.readFile(mdFilePath, 'utf-8', function(err, data) {
      if(err) {
        console.log(err)
      } else {
        var frontMatter = self.findFrontMatter(data)

        console.log('\n\n\nstreetlight', frontMatter)
        if(frontMatter['vsync']) {
          delete frontMatter['vsync']

          var embedValue = frontMatter['embed']

          if(embedValue) {
            // check if embed is custom s3 link
            var client = GHelper.createClient(s3Data)

            var VHelper = require('./video-helper')(client)

            if(VHelper.isS3Link(embedValue, s3Data)) {
              // if true then delete from s3
              delete frontMatter['embed']
              VHelper.removeVideo(embedValue, s3Data, client, sender, callback)
            }
          }
        }

        var newContent = self.replaceFrontMatter(data, frontMatter)

        fs.writeFile(mdFilePath, newContent, function(err) {
          if(err) {
            console.log(err)
          } else {
            console.log('vsync removed')
            NM_OTHER_PAGE_COUNT++
  ...
  */
   ...
   */
  cleanUpUnusedVsyncAndEmbed: function(mdFilePath, s3Data, client, sender, callback) {
    var self = this
    fs.readFile(mdFilePath, 'utf-8', function(err, data) {
      if(err) {
        console.log(err)
        ...
        var frontMatter = self.findFrontMatter(data)
        console.log('FRONTMATTER', frontMatter['vsync'])
        if(frontMatter && frontMatter['vsync'] !== undefined) {

          var filePathWithoutFile = FHelper.filePathWithoutFile(mdFilePath)

          dir.files(filePathWithoutFile, function(err, files) {
            console.log("bello")
            if (err) throw err
            ...
            if(videos.length === 0) {
              console.log("cello")
              self.deleteFromS3AndCleanFrontMatter(mdFilePath, s3Data, client, sender, callback)
            } else {
              NM_OTHER_PAGE_COUNT++
              GHelper.checkIfDone(callback)
            }
